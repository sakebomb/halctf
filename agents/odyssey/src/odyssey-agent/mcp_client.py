...
async def _get_challenge(ctf_hint: str, challenge_hint: str) -> Optional[Dict]:
    """
    Fetch challenge details including attachments via MCP get_challenge tool.
    Returns challenge object or None.
    """
    endpoint = os.environ.get("MCP_ENDPOINT") or "http://127.0.0.1:9000/mcp/"
    if not endpoint.rstrip("/").endswith("/mcp"):
        endpoint = endpoint.rstrip("/") + "/mcp/"
    ...
    try:
        ...
        print(f"[MCP] SDK import failed ({e})", flush=True)
        return None
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                async def call(name: str, **arguments):
                    try:
                        ...
                        print(f"[MCP] call {name} failed: {e}", flush=True)
                        return None

                # Try get_challenge with ctf + challenge hints
                result = await call("get_challenge", ctf=ctf_hint, challenge=challenge_hint)
                if result:
                    return result if isinstance(result, dict) else None

                return None
    except Exception as e:
        print(f"[MCP] get_challenge error: {e}", flush=True)
        return None


def get_challenge_details(ctf_hint: str, challenge_hint: str) -> Optional[Dict]:
    """Synchronous wrapper for getting challenge details."""
    try:
        return asyncio.run(_get_challenge(ctf_hint, challenge_hint))
    except Exception as e:
        print(f"[MCP] get_challenge_details error: {e}", flush=True)
        return None
