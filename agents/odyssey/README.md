# Odyssey Agent

**CTF Category:** [Fill in]
**Challenges:** [Fill in]
**Points:** [Fill in]
**Latest Version:** [Fill in]

## Overview

[Brief description of what this agent solves]

## Build

```bash
cd agents/odyssey/src/
docker build -t odyssey-agent:latest .
docker save odyssey-agent:latest > ../builds/odyssey-agent-vN.tar
```

## Usage

```bash
# Load pre-built image
docker load < agents/odyssey/builds/odyssey-agent-vN.tar

# Run (platform provides environment variables)
docker run --rm -it odyssey-agent:vN
```

## Architecture

[Brief description of solver architecture]

## Versions

See [VERSIONS.md](./builds/VERSIONS.md) for changelog.

## See Also

- [Write-up](../../docs/writeups/[challenge-name].md)
- [Logs](../../logs/odyssey/)
